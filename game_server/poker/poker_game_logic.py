...
from .hand_evaluation import (
    ...
    is_pair,
    get_high_cards,
    get_four_of_a_kind_high_card,
    get_full_house_high_cards,
    get_three_of_a_kind_high_card,
    get_two_pair_high_cards,
    get_pair_high_cards,
)

# Possible win options and their probabilities
win_options = ["All in", "Showdown", "Forfeit", "Fold"]
wo_probabilities = [0.10, 0.40, 0.05, 0.45]

# Possible winning poker hands and their probabilities
...
class Player:
    """
    Represents the player in a poker game.
    """

    def __init__(self, player_id, name, behavior):
        """
        Initialize player with player_id, name and behavior
        """
    ...
    def deal_hole_cards(self, deck):
        """
        Deal two hole cards to this player from the deck
        """
    ...
    def __str__(self):
        """
        Returns string details of the player. For debugging purposes.
        """
...
def generate_player_id(name):
    """
    Generate a 25 character unique player ID based on player name. Assigned ID
    The generated ID will be the same for players it is assigned to for every
    API request.
    """
...
def create_player_pool(num_players):
    """
    Create players depending on parameter for num_players
    """
    player_pool = {}
...
def draw_cards(deck, num):
    """
    Function for drawing community, river and turn cards
    """
...
def generate_hole_and_community_cards(player_1, player_2):
    """
    Generate cards to be played for the game. Assigns hole cards for both players.
    """
...
def determine_winner(
    hand1, hand2, hand1_high_cards, hand2_high_cards, hole_cards_p1, hole_cards_p2
):
    """
    Determine winner between two poker hands.
    """
    if hand_values[hand1] > hand_values[hand2]:
        ...
        hole_card_values_p1 = sorted(
            [rank_values[card[:-1]] for card in hole_cards_p1], reverse=True
        )
        hole_card_values_p2 = sorted(
            [rank_values[card[:-1]] for card in hole_cards_p2], reverse=True
        )
...
def generate_winnings(winner, player1, player2, player1_bets, player2_bets):
    """
    Calculates value of netwin for both players, total pot, and house winnings from rake
    """
    rake = 5
    initial_bets = 10
...
def place_bets(player1, player2, win_option):
    """
    Place bets for the players based on the win option.

    :param player1: Player 1 object
    :param player2: Player 2 object
    :param win_option: Win option ("All in", "Showdown", "Forfeit", "Fold")
    :return: Player 1's bets and player 2's bets
    """
    if win_option == "All in":
        ...
        player2_bets = min(
            player1_bets, player2.chips
        )  # Match player1's bet or go all-in if player2 has less chips
...
def generate_employee_id():
    """
    Generate a 6-character unique employee ID
    """
...
def game_details(rounds):
    ...
    """
    Sets game details for the api response.
    Includes:
    - game id
    - dealer for current game
    - datetime start and end of poker game 
    """
    ...
    datetime_end = (
        datetime_start + datetime.timedelta(minutes=random.randint(5, 10) * rounds)
...
def generate_poker_hand():
    """
    Simulate a poker game and return detailed game data in a dictionary format.

    :return: Dictionary containing game data for a poker hand
    """
    ...
    hole_cards_p1, hole_cards_p2, community_cards = generate_hole_and_community_cards(
        player_1, player_2
    )
    ...
    player1_netwin, player2_netwin, house_earnings, total_pot, rake, initial_bets = (
        generate_winnings(winner, player_1, player_2, player1_bets, player2_bets)
    )
    ...
    game_data = {
        ...
        "players": [
            {
                ...
                "holeCards": [
                    {"rank": rank_values[card[:-1]], "suit": card[-1]}
                    for card in player_1.hole_cards
                ],
            ...
            },
            {
                ...
                "holeCards": [
                    {"rank": rank_values[card[:-1]], "suit": card[-1]}
                    for card in player_2.hole_cards
                ],
        ...
        "communityCards": {
            ...
            "turn": {
                "rank": rank_values[community_cards[3][:-1]],
                "suit": community_cards[3][-1],
            },
            "river": {
                "rank": rank_values[community_cards[4][:-1]],
                "suit": community_cards[4][-1],
            },
        },
    }
